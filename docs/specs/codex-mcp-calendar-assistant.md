...
- `src/server/calendar/calendar-verification-subagent.ts` verifies direct Google Calendar writes by reading the event back.
...
- Always: direct MCP calendar registration or changes must create an action record, run verification, and create an assistant message log.
...
- Never: let Codex apply calendar actions without a tracked action record, verification result, and change log.
...
- Calendar verification subagent confirms the resulting Google event title, time range, and timezone.
